...
try {
  connect-VIServer $vcserver -User "opsrampuser@vsphere.local" -Password "P@ssw0rdMelco" -ErrorAction Stop |  Out-File -FilePath "C:\scripts\Vmware\vcsconnect_logs.txt"
  
}
catch {
  Write-Output "Incorrect Username and/or Password. Please check the VCS_error file for review" -ForegroundColor Red
  Write-Output $Error | Out-File -FilePath "C:\Scripts\Vmware\VCS_error.txt" -Append
  throw $_
}
...
try {
  ...
  $esx |  out-file -FilePath "C:\scripts\Vmware\vmhost_info.txt"
}
catch {
    Write-Output "An error occured while fetching the host information" | Out-File -FilePath "C:\Scripts\Vmware\host_fetch_error.txt"
    Write-Output $Error | Out-File -FilePath "C:\Scripts\Vmware\host_fetch_error.txt" -Append
  } 
...
$Reportesx +=$allinfo
  ...
  Write-Output "An error occured while fetching the host information" | Out-File -FilePath "C:\Scripts\Vmware\host_metricsfetch_error.txt"
}

finally {
  Write-Output $Error | Out-File -FilePath "C:\Scripts\Vmware\host_metricsfetch_error.txt"
}
...
$Attachment = "$path\healthcheck.htm"
